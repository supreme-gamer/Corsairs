...
ref BI_CommandEndChecking()
{
	...
	case "BI_SailTo":
		...
		if (bBettaTestMode || csmCA(pchar, "CSM.SailToEffectRadius")) // --> CSM
		{
...
*/		
	...
	case "BI_SailTo":
		...
		// boal 09.02.2004 -->
	    if (bDisableMapEnter && !bBettaTestMode && !CheckAttribute(pchar, "questTemp.Sharlie.Lock") && !CheckAttribute(pchar, "GenQuest.MapClosedNoBattle") && !csmCA(pchar, "CSM.FreeSailing")) // --> CSM //Jason
	    {
           PlaySound("interface\knock.wav");
	       break;
	    }
	    if (!CheckEnemyCompanionDistance2GoAway(false) && !bBettaTestMode && !csmCA(pchar, "CSM.FreeSailing")) // --> CSM // компаньон в беде
	    {
           PlaySound("interface\knock.wav");
	       break;
	    }
		...
		{ // приплыть в локатор с именем locName
			...
			if (MOD_SKILL_ENEMY_RATE >= 2 && !bBettaTestMode && !csmCA(pchar, "CSM.FreeSailing")) // --> CSM
            {
		...
		{ // догнать перса с индексом targetNum
            ...
            if (GetRelation(sti(pchar.index), targetNum) != RELATION_ENEMY || CheckAttribute(&Characters[targetNum], "CanBeSailTo") || bBettaTestMode || csmCA(pchar, "CSM.FreeSailing")) // --> CSM
            {
			    if( !IsEntity(&SailToFader) ) {SeaAI_SailToCharacter(targetNum); }
			}
			else
...
void BI_SetPossibleCommands()
{
	...
	if(mainIdx==chIdx)
	{
		if (csmCA(pchar, "CSM.KillEmAll")) // --> CSM
		{
...
void SetShipPictureDataByShipTypeName(string sType)
{
	...
	switch (sType)
	{
		case "tartane":					i = 0; break;
		case "WarTartane":				i = 2; break;
		case "barque":					i = 4; break;
		case "schooner":				i = 6; break;
		case "barkentine":				i = 8; break;
		case "shnyava":					i = 10; break;
		case "fleut":					i = 12; break;
		case "caravel":					i = 14; break;
		case "pinnace":					i = 16; break;
		case "caracca":					i = 18; break;
		case "Navio":					i = 20; break;
		case "EastIndiaMan":			i = 22; break;
		case "LuggerQuest":				i = 24; break;
		case "lugger":					i = 26; break;
		case "sloop":					i = 28; break;
		case "brigantine":				i = 30; break;
		case "brig":					i = 32; break;
		case "schooner_w":				i = 34; break;
		case "galeon_l":				i = 36; break;
		case "corvette":				i = 38; break;
		case "XebekVML":				i = 40; break;
		case "Polacre":					i = 42; break;
		case "Galeon_h":				i = 44; break;
		case "frigate":					i = 46; break;
		case "frigate_h":				i = 48; break;
		case "lineship":				i = 50; break;
		case "warship":					i = 52; break;
		case "battleship":				i = 54; break;
		case "manowar":					i = 56; break;
		case "manowar_e":				i = 58; break;
		case "Mayfang":					i = 60; break;
		case "Mirage":					i = 62; break;
		case "Valciria":				i = 64; break;
		case "Corvette_quest":			i = 66; break;
		case "ArabellaShip":			i = 68; break;
		case "Flyingdutchman":			i = 70; break;
		case "Flyingdutchman_l":		i = 72; break;
		case "Santisima":				i = 74; break;
		case "Polacre_q":				i = 76; break;
		// 2do: Лень делать для новых кораблей уникальные иконки, пусть юзаются дефолтные
		case "BlackPearl":				i = 72; break;
		case "ManowarV":				i = 50; break;
		case "SoleyRu":					i = 58; break;
		case "QAR":						i = 68; break;
		case "Rossiya":					i = 68; break;
		case "Surprise":				i = 68; break;
		case "Constitution":			i = 68; break;
		case "Speedy":					i = 32; break;
		case "Corvette_questCoAS":		i = 66; break;
		case "ArabellaShipCoAS":		i = 68; break;
		case "FrigateQueen":			i = 68; break;
		case "BrigSW":					i = 64; break;
		case "BrigQueen":				i = 64; break;
		case "LuggerQuestCoAS":			i = 24; break;
		case "Essex":					i = 68; break;
		case "LeSuperbe":				i = 68; break;
		case "Centurion":				i = 68; break;
		case "Mordaunt":				i = 68; break;
		case "Oxford":					i = 68; break;
		case "Resolution":				i = 56; break;
		case "Prince":					i = 58; break;
		case "Nonsuch":					i = 62; break;
	}

	BI_intNRetValue[0] = i;
	BI_intNRetValue[1] = i + 1;
...
void SetParameterData()
{
	...
	BattleInterface.navigation.speedOutYOffset			= RecalculateVIcon(makeint(202 * fHtRatio));
	...
	BattleInterface.navigation.chargePos				= RecalculateHIcon(makeint(-30 * fHtRatio))+","+RecalculateVIcon(makeint(270 * fHtRatio));
	...
	BattleInterface.imageslist.textinfoback4.pos = (sti(showWindow.right) - RecalculateHIcon(98 * fHtRatio)) + "," + (sti(showWindow.top) + RecalculateVIcon(350 * fHtRatio)) + "," + (sti(showWindow.right) - RecalculateHIcon(50 * fHtRatio)) + "," + (sti(showWindow.top) + RecalculateVIcon(398 * fHtRatio));
	// CSM <--
	...
	BattleInterface.textinfo.Location.pos.y = RecalculateVIcon(makeint(222 * fHtRatio));
	BattleInterface.textinfo.Location.text = GetCurLocationName();//GetBILocationName();	// Display MoorName or region name in location
	...
	BattleInterface.textinfo.Ammo.pos.x = sti(showWindow.right) - RecalculateHIcon(makeint(135 * fHtRatio));
	BattleInterface.textinfo.Ammo.pos.y = RecalculateVIcon(makeint(405 * fHtRatio));
	...
	BattleInterface.textinfo.Powder.pos.x = sti(showWindow.right) - RecalculateHIcon(makeint(75 * fHtRatio));
	BattleInterface.textinfo.Powder.pos.y = RecalculateVIcon(makeint(405 * fHtRatio));
	...
	BattleInterface.LineInfo.speed.begin.y = RecalculateVIcon(makeint(333 * fHtRatio));
	...
	BattleInterface.LineInfo.Ammo.begin.y = RecalculateVIcon(makeint(430 * fHtRatio));
	...
	if (!bSeaActive || mchar.location == mchar.SystemInfo.CabinType) // спим в каюте
	{
		for (i=0; i<COMPANION_MAX; i++)
		{
			...
			if(CheckAttribute(chref, "Ship.HP_penalty"))
			{
				...
				else RealShips[sti(chref.ship.type)].HP = 0;
...
float fSailState; // Dolphin -> для звука
	if(ControlName == "Ship_TurnLeft") PlaySound("ship\turn_left_"+rand(4)+".ogg");
	if(ControlName == "Ship_TurnRight") PlaySound("ship\turn_right_"+rand(4)+".ogg");
	// <-	

	switch(ControlName)
	{
		case "hk_charge1":
			csmStopSND(csmINT[1], -1);

            csmINT[1] = csmPS("interface\_balls", 0);
		...
		case "hk_charge2":
            ...
            csmINT[1] = csmPS("interface\_grapes", 0);
		...
		case "hk_charge3":
            ...
            csmINT[1] = csmPS("interface\_chain", 0);
		...
		case "hk_charge4":
            ...
            csmINT[1] = csmPS("interface\_bombs", 0);
