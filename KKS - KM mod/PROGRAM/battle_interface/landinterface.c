...
//	objLandInterface.textinfo.datatext.scale = 0.9;
	...
	objLandInterface.textinfo.AltInfo4.pos.x = sti(showWindow.left) + RecalculateHIcon(384 * fHtRatio); 
	...
	if(CheckAttribute(loadedLocation,"fastreload")) // подсказка только там, где она нужна
	{
		...
		objLandInterface.textinfo.AltInfo.pos.x = sti(showWindow.left) + RecalculateHIcon(320 * fHtRatio); 
		...
		objLandInterface.textinfo.AltInfo1.pos.x = sti(showWindow.left) + RecalculateHIcon(343 * fHtRatio); 
		...
		objLandInterface.textinfo.AltInfo2.pos.x = sti(showWindow.left) + RecalculateHIcon(365 * fHtRatio); 
...
void BLI_SetPossibleCommands()
{
	...
	if(chrMode==0)
	{
		...
		if (bTmpBool && !csmCA(pchar, "CSM.FastTravelEnemy")) // --> CSM // все еще можно переходить, провер¤ем город враг
		{
		    string sNation = Colonies[FindColony(loadedLocation.fastreload)].nation;
			if (sNation != "none")
			{
	...
	if( CheckAttribute(pchar,"activeItem") && pchar.activeItem!="" )
	{
		...
		{
			//objLandInterface.Commands.TakeItem.enable = true; // Jason: баг с двойным вз¤тием через быструю команду+интерфейс
		}
		else
		{
			objLandInterface.Commands.PlaceItem.enable = true;
