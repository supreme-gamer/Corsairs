...
*/							
	...
	worldMap.shipSpeedOppositeWind = 0.5;
	worldMap.shipSpeedOverWind = 1.0;
	...
	worldMap.wdmCameraY = 160.0;				//Высота камеры
	worldMap.wdmCameraRotLock = 1;				//Заблокировать вращение камеры
	//Enemy ship	
	worldMap.enemyshipViewDistMin = 175.0;		//Растояние на котором корабль начинает исчезать
	worldMap.enemyshipViewDistMax = 200.0;		//Растояние на котором корабль исчезает полностью
    worldMap.enemyshipDistKill = 3000;          // homo 07/10/06
    //worldMap.enemyshipDistKill = 150.0;			//Расстояние на котором убиваем корабль
	worldMap.enemyshipBrnDistMin = 90.0;		//Минимальное растояние на котором рожается корабль
	worldMap.enemyshipBrnDistMax = 140.0;		//Максимальное растояние на котором рожается корабль
	//storm
	worldMap.stormViewDistMin = 170.0;			//Растояние на котором шторм начинает исчезать
	worldMap.stormViewDistMax = 200.0;			//Растояние на котором шторм исчезает полностью
	worldMap.stormDistKill = 250.0;				//Расстояние на котором убиваем шторм
	worldMap.stormBrnDistMin = 160.0;			//Минимальное растояние на котором рожается шторм
	worldMap.stormBrnDistMax = 180.0;			//Максимальное растояние на котором рожается шторм
	worldMap.stormZone = 80.0;// 120					//Общий радиус действия шторма
