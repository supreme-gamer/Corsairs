...
bool wdmCreateFollowShipByIndex(float kSpeed, int index, ref encID, int timeOutInDays)
{
	...
	if (csmCA(pchar, "CSM.SkipShips") || klas || (GetNationRelation2MainCharacter(iNation) != RELATION_ENEMY)) // --> CSM
	{
...
bool wdmIsEncounterEnemy(int index) 
{
	...
	bool isProfessional = (IsCharacterPerkOn(pchar, "SeaDogProfessional")) && (GetCharacterShipClass(pchar) < 5);
	...
	// пираты не нападают без грозы морей при следующих условиях - 
	ref Kpsld;
	int Kpan;
	int iClass = 6;
    for(int i = 1; i < COMPANION_MAX; i++)
	{
		Kpan = GetCompanionIndex(PChar, i);
		if(Kpan >= 1)
		{
			Kpsld = GetCharacter(Kpan);
			 if (GetCharacterShipClass(Kpsld) <= iClass)
			 {
			  iClass = GetCharacterShipClass(Kpsld);
			 }
		}
	}
    if (GetCharacterShipClass(pchar) <= 2 && iEncType == ENCOUNTER_TYPE_PIRATE_SMALL)
	{ // если ГГ на 2 классе то контра не суются
		return false;
	}
	if (GetCharacterShipClass(pchar) <= 4 && GetCompanionQuantity(pchar) >= 2 && iClass <= 3 && iEncType == ENCOUNTER_TYPE_PIRATE_SMALL)
	{  // если  ГГ на 4 классе + есть копманьон от 3 класса - контра не суётся
		return false;
	}
	  if (GetCharacterShipClass(pchar) < 2 && iEncType == ENCOUNTER_TYPE_PIRATE_MEDIUM)
	{ // если ГГ на 1 классе то крысы не суются
		return false;
	}
	if (GetCharacterShipClass(pchar) <= 3 && GetCompanionQuantity(pchar) >= 3 && iClass <= 2 && iEncType == ENCOUNTER_TYPE_PIRATE_MEDIUM)
	{   // если ГГ на 3 классе + 2 компаьнона то крыса не сутся
		return false;	
	}
	if (GetCharacterShipClass(pchar) < 2 && GetCompanionQuantity(pchar) >= 4 && iClass < 2 && iEncType == ENCOUNTER_TYPE_PIRATE_LARGE) // iClass <= 4
	{  // ГГ 1 класс + компаньона 3 и 1 класса. рейдер мимо.
		return false;		
	}
