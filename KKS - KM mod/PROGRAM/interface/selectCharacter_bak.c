...
string SetSetupInfo(string type)
{
	...
	switch (type)
	{
		...
		break;
		case "HeroType_4":
			setupInfo = "Постоянные амурные приключения и вызванные ими дуэли не прошли даром: персонаж расходует на 40% меньше времени на перезарядку личного оружия." + NewStr() +
