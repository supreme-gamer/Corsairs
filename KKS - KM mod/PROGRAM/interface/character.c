...
void IncreaseSkill(int _add)
{
    ...
    else
    {
	    ...
	    { //для начала игры пересчет скилов
		    ...
		    InitStartParam(xi_refCharacter); //Serj
		    SetEnergyToCharacter(xi_refCharacter);
	    }
    }
	SetVariable();
...
void ShowMsgMenu()
{
	...
	SetFormatedText("MSG_WINDOW_TEXT", "Для изменения параметров героя укажите характеристику и стрелками перераспределите очки. Умения автоматически пересчитаются от выбранных PIRATES. Если вы хотите оставить стандартные параметры героя и уникальные значения прокачки его навыков(без пересчета), то закройте форму не внося изменений."); //Serj
