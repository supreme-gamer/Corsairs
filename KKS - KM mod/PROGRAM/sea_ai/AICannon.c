...
float Cannon_GetRechargeTime()
{
	...
	{
		if (csmCA(pchar, "CSM.MachineGunCannons") && sti(aCharacter.index) == csmGMI())
			return SM_AF_ModifyReloadTime(aCharacter, 1.0);
		else
			return SM_AF_ModifyReloadTime(aCharacter, fMultiply * fReloadTime * fCannonSkill);
	}
	// Sticks <--
	
	// --[ CSM
	if (csmCA(aCharacter, "CSM.MachineGunCannons"))
		return 1.0;
	else
...
float Cannon_GetFireTime()
{
	...
	if (refBaseShip.Name == ShipsTypes[SHIP_FORT].Name) { return frnd() * 10.0; }   // иначе пулеметный залп
	...
	// Sticks AltFire -->
	if (CheckAttribute(pchar, "CSM.AltFire"))
		return SM_AF_ModifyFireTime(aCharacter, fFireTime);
	// Sticks <--
