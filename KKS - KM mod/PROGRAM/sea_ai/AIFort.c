	...
	for (int i=0; i<iNumReloads; i++)
	{
		...
		if (CheckAttribute(arLocator,"fort"))
		{
			...
			if (bFortRessurect) // восстановили
			{
				...
				rCharacter.Ship.Crew.Quantity = 600 + rand(1800);
			}
			...
			{
			    SetCharacterGoods(rCharacter, GOOD_BALLS,  100000  + rand(200));// boal fix не токо бомбы!
				SetCharacterGoods(rCharacter, GOOD_GRAPES,  25000  + rand(200));
				SetCharacterGoods(rCharacter, GOOD_KNIPPELS,  25000  + rand(200));
			    SetCharacterGoods(rCharacter, GOOD_POWDER, 200000 + rand(200));
			}
			else
			{
			    SetCharacterGoods(rCharacter, GOOD_BALLS,  100 + rand(200));
			...
			SetCharacterGoods(rCharacter, GOOD_RUM,   500 + rand(150));

			// тут золото по квестам -->
			SetCharacterGoods(rCharacter, GOOD_GOLD,   rand(125));
			SetCharacterGoods(rCharacter, GOOD_SILVER,   rand(250));
...
{
	if (!CheckAttribute(rCharacter, "Ship.Cannons.Type")) { rCharacter.Ship.Cannons.Type = CANNON_TYPE_CANNON_LBS48; }
	...
	rCharacter.Ship.Cannons.Charge.Type = GOOD_BALLS;  // всегда бомбы   boal
}
