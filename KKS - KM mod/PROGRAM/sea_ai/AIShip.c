...
float Ship_MastDamage()
{
	...
	// --[ CSM
	if (csmCA(rCharacter, "CSM.MastNoFall") || csmCA(csmGRS(rCharacter), "CSM.MastNoFall"))
		fDamage = 0.0;
	// CSM ]--
...
{
	// Sticks AltFire -->
	if (CheckAttribute(pchar, "CSM.AltFire"))
		SM_AF_Continue = !SM_AF_Continue;
	// Sticks <--
...
void Ship_OnBortReloaded()
{
	...
	// Sticks AltFire -->
	if (CheckAttribute(pchar, "CSM.AltFire"))
		SM_AF_FireContinue(); // чтоб не закликивать до ...
	// Sticks <--
}
...
void Ship_BortReloadEvent()
{
	...
	if (sti(aCharacter.Tmp.BortsReloaded.Event))
	{
		...
		{
			// --> Sticks
			if (!CheckAttribute(pchar, "CSM.AltFire")) { // не комментировать каждую заряженную пуху
...
void Ship_ApplyHullHitpoints(ref rOurCharacter, float fHP, int iKillStatus, int iKillerCharacterIndex)
{
	...
	if (!csmCA(pchar, "CSM.NoShipHullPenalty")) // --> CSM
	{
...
void Ship_HullHitEvent()
{
	...
	if (!csmCA(rOurCharacter, "CSM.NoCannonsBoom")) // --> CSM
...
void Ship_CheckMainCharacter()
{
    ...
    if (locTmpTime > 9)
	{
		...
		{ */
		    CalcLocalTime(2);
      		BattleInterface.textinfo.Date.text = /*XI_convertString("Date:")+*/GetQuestBookData(); // --> CSM
		//}
	}
	// boal время в море идет!!! <--
			...
			if (!bSkipTarget)
			{
				...
				if (sti(rCharacter.SeaAI.Task) == AITASK_RUNAWAY)
				{
					...
					*/
					...
					if (!csmCA(pchar, "CSM.DropGoodsOff") && sti(rCharacter.Ship.SP) > 50 && rand(100) > 80)  // не выкидывать, если не убежать, а то нет смысла брать приз
					{
...
void SetTextureForShip(aref refBaseShip, aref rCharacter)
{
	...
	// --[ CSM --> В ЧМ можно:
	// общие текстуры для кораблей кидать в папку "RESOURCE\Textures\Ships\Textures"
	// общие текстуры для конкретного корабля кидать в папку "RESOURCE\Textures\Ships\<папка_корабля>", а в папки hull* помещать только различающиеся элементы
	// а так же можно на всё это забить и поступать как раньше ^^

	string sPathRoot = "ships\Textures\";
	string sPathShip = "ships\" + refBaseShip.name + "\";
	string sPathHull = sPathShip + "hull" + refBaseShip.ship.upgrades.hull + "\";

	SetTexturePath(0, sPathRoot);
	SetTexturePath(1, sPathShip);
	SetTexturePath(2, sPathHull);
	// CSM ]--
}
