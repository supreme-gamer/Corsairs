...
// boal 20.01.2004 -->
	...
	{
		case "trade":
			rFantom.Money = (10 - iSClass) * 200 + rand((10 - iSClass) * 800);
			return;
		break;
		case "war":
			rFantom.Money = (10 - iSClass) * 80 + rand((10 - iSClass) * 350);
			return;
		break;
		case "pirate":
		    rFantom.Money = (10 - iSClass) * 160 + rand((10 - iSClass) * 500);
			return;
		break;
	}
// boal 20.01.2004 <--
...
void Fantom_SetBalls(ref rFantom, string sFantomType)
{
    ...
    // boal 20.01.2004 -->
	Fantom_SetCharacterGoods(rFantom, GOOD_BALLS,    	MakeInt(300 * fKBalls + rand(MakeInt(10 * fKBalls))), 0);
	Fantom_SetCharacterGoods(rFantom, GOOD_BOMBS,    	MakeInt(200  * fKBalls + rand(MakeInt(20 * fKBalls))), 0);
	Fantom_SetCharacterGoods(rFantom, GOOD_KNIPPELS, 	MakeInt(200  * fKBalls + rand(MakeInt(10 * fKBalls))), 0);
	Fantom_SetCharacterGoods(rFantom, GOOD_GRAPES,   	MakeInt(150  * fKBalls + rand(MakeInt(10 * fKBalls))), 0);
	Fantom_SetCharacterGoods(rFantom, GOOD_POWDER,   	MakeInt(450 * fKBalls + rand(MakeInt(30 * fKBalls))), 0);

	Fantom_SetCharacterGoods(rFantom, GOOD_SAILCLOTH, 	MakeInt(25 	* fKBalls + rand(MakeInt(10 * fKBalls))), 0);
	Fantom_SetCharacterGoods(rFantom, GOOD_PLANKS, 		MakeInt(25 	* fKBalls + rand(MakeInt(20 * fKBalls))), 0);

    Fantom_SetCharacterGoods(rFantom, GOOD_FOOD, 		MakeInt(40 	* fKBalls + rand(MakeInt(26 * fKBalls))), 0);
    Fantom_SetCharacterGoods(rFantom, GOOD_WEAPON, 		MakeInt(30  * fKBalls + rand(MakeInt(13 * fKBalls))), 0);
    Fantom_SetCharacterGoods(rFantom, GOOD_RUM, 		MakeInt(20  * fKBalls + rand(MakeInt(10 * fKBalls))), 0);
    Fantom_SetCharacterGoods(rFantom, GOOD_MEDICAMENT, 	MakeInt(25  * fKBalls + rand(MakeInt(13 * fKBalls))), 0);
    // boal 20.01.2004 <--
}
			...
			if (CheckAttribute(rFantom, "RealEncounterType"))
			{
				switch (sti(rFantom.RealEncounterType))
				{
					...
					case ENCOUNTER_TYPE_MERCHANT_EXPEDITION 	:
						...
						// if(Nation == SPAIN)
						iFinish = GOOD_CANNON_3 - iStart - 1; // золото и серебро только у испанцев !!!!
