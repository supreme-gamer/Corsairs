...
void CreateColonyCommanders()
{
	...
	for (i=0; i<MAX_COLONIES; i++)
	{
		...
		UpdateRelations();
  		...
  		characters[iChar].Fort.Cannons.Charge.Type = GOOD_BALLS;

        SetCrewQuantityFull(&characters[iChar]);
        
		characters[iChar].Fort.Cannons.Type.1 = CANNON_TYPE_CANNON_LBS48;
		characters[iChar].Fort.Cannons.Type.2 = -1;
		characters[iChar].Fort.Cannons.Type.3 = -1;

		if(sti(colonies[i].fort) > 1)
		{
			characters[iChar].Fort.Cannons.Type.2 = CANNON_TYPE_CANNON_LBS48;
		}
...
void TWN_FightInTown()
{
    ...
    {
		csmDA(pchar, "CSM.LootCollector.CanBeRun"); // --> CSM
