...
{
	if (csmCA(pchar, "CSM.PaymentMultipler") && stf(pchar.CSM.PaymentMultipler) <= 0.0)
		return 0;
    ...
    if (CheckAttribute(Npchar, "Payment") && makeint(Npchar.Payment) == true)
    {
		...
		if (csmCA(pchar, "CSM.PaymentMultipler") && stf(pchar.CSM.PaymentMultipler) > 0.0)
			sum = makeint(sum * stf(pchar.CSM.PaymentMultipler));
...
int GetSalaryForShip(ref chref)
{
	...
	// Sticksъ >
	if (csmCA(pchar, "CSM.PaymentMultipler"))
		nPaymentQ = makeint(nPaymentQ * stf(pchar.CSM.PaymentMultipler));
	// < Sticksъ
...
*/
			nNeedCrew = makeint(abs(REPUTATION_NEUTRAL - sti(pchar.reputation.nobility))/MOD_SKILL_ENEMY_RATE + sti(pchar.rank)/MOD_SKILL_ENEMY_RATE + rand(sti(pchar.rank)/2) + drand(100 + MOD_SKILL_ENEMY_RATE));
...
{
	if (csmCA(pchar, "CSM.NoShipMutinies"))
		return;
	...
	if (state == "after" && CheckAttribute(Pchar, "Partition.before.Money"))
	{
		...
		{  // Делим бабки	
			if(bPartitionSet)
			{
				...
				// Sticksъ >
				if (csmCA(pchar, "CSM.PaymentMultipler"))
					Pchar.Partition.MonthPart.Gower = stf(Pchar.Partition.MonthPart.Gower) * stf(pchar.CSM.PaymentMultipler));
				// < Sticksъ
