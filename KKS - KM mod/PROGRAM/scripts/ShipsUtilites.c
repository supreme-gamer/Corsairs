...
void SetShipyardStore(ref NPChar)
{
	...
	// --[ CSM
    if (bBettaTestMode || csmCA(pchar, "CSM.AllShipsInTrade"))
    {
    ...
    if(sti(NPChar.nation) == ENGLAND || sti(NPChar.nation) == FRANCE || sti(NPChar.nation) == SPAIN || sti(NPChar.nation) == HOLLAND)
	{
		...
		{
			iTest_ship = rand(1);
			if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_TARTANE, NPChar), "ship1");
			if (iTest_ship == 1) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_WAR_TARTANE, NPChar), "ship1");
		}

		//Второй слот
		{
			iTest_ship = rand(2);
			if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_WAR_TARTANE, NPChar), "ship2");
			if (iTest_ship == 1) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_LUGGER, NPChar), "ship2");
			if (iTest_ship == 2) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_CAREERLUGGER, NPChar), "ship2");
		}
		...
		//Третий слот
		{
			...
			{
				iTest_ship = rand(2);
				if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_LUGGER, NPChar), "ship3");
				if (iTest_ship == 1) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_SLOOP, NPChar), "ship3");
				if (iTest_ship == 2) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_BARQUE, NPChar), "ship3");
			}
			else
			{
				iTest_ship = rand(2);
				if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_BARQUE, NPChar), "ship3");
				if (iTest_ship == 1) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_SCHOONER, NPChar), "ship3");
				if (iTest_ship == 2) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_BARKENTINE, NPChar), "ship3");
			}
		}
		...
		//Четвертый слот
		{
			...
			{
				iTest_ship = rand(2);
				if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_BARKENTINE, NPChar), "ship4");				
				if (iTest_ship == 1) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_SCHOONER, NPChar), "ship4");	
				if (iTest_ship == 2) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_SHNYAVA, NPChar), "ship4");
			}
			else
			{
				iTest_ship = rand(1);
				if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_BARKENTINE, NPChar), "ship4");
				if (iTest_ship == 1 && sti(NPChar.nation) == ENGLAND) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_BRIG, NPChar), "ship4");				
				if (iTest_ship == 1 && sti(NPChar.nation) == SPAIN) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_BRIGANTINE, NPChar), "ship4");
				if (iTest_ship == 1 && sti(NPChar.nation) == FRANCE) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_SCHOONER_W, NPChar), "ship4");
				if (iTest_ship == 1 && sti(NPChar.nation) == HOLLAND) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_FLEUT, NPChar), "ship4");
			}
		}

		//Пятый слот
		{
			if ((sti(PChar.rank) >= 10) && (sti(PChar.rank) < 15))
			{
				iTest_ship = rand(2);						
				if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_BRIGANTINE, NPChar), "ship5");	
				if (iTest_ship == 1) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_SHNYAVA, NPChar), "ship5");			
				if (iTest_ship == 2) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_SCHOONER, NPChar), "ship5");
			}

			if (sti(PChar.rank) >= 15)
			{
				iTest_ship = rand(2);						
				if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_CARAVEL, NPChar), "ship5");				
				if (iTest_ship == 1) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_CORVETTE, NPChar), "ship5");
				if (iTest_ship == 2 && sti(NPChar.nation) == ENGLAND) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_CORVETTE, NPChar), "ship5");				
				if (iTest_ship == 2 && sti(NPChar.nation) == FRANCE) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_CORVETTE, NPChar), "ship5");				
				if (iTest_ship == 2 && sti(NPChar.nation) == SPAIN) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_GALEON_L, NPChar), "ship5");
				if (iTest_ship == 2 && sti(NPChar.nation) == HOLLAND) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_PINNACE, NPChar), "ship5");
			}
		}
		...
		//Шестой слот
		{
			...
			{
				iTest_ship = rand(2);						
				if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_CARACCA, NPChar), "ship6");				
				if (iTest_ship == 1) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_POLACRE, NPChar), "ship6");
				if (iTest_ship == 2)
				{
					iTest_ship = rand(1);
					if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_PINNACE, NPChar), "ship6");
				}
			}
			...
			{
				iTest_ship = rand(1);				
				if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_POLACRE, NPChar), "ship6");
				if (iTest_ship == 1 && sti(NPChar.nation) == FRANCE) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_XebekVML, NPChar), "ship6");				
				if (iTest_ship == 1 && sti(NPChar.nation) == SPAIN) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_XebekVML, NPChar), "ship6");
				if (iTest_ship == 1 && sti(NPChar.nation) == ENGLAND) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_FRIGATE, NPChar), "ship6");
				if (iTest_ship == 1 && sti(NPChar.nation) == HOLLAND) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_NAVIO, NPChar), "ship6");				
			}
		}
		...
		//Седьмой слот
		{
			...
			{
				iTest_ship = rand(3);						
				if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_FRIGATE, NPChar), "ship7");
				if (iTest_ship == 1) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_FRIGATE_H, NPChar), "ship7");
				if (iTest_ship == 2) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_NAVIO, NPChar), "ship7");
				if (iTest_ship == 3 && sti(NPChar.nation) == FRANCE) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_FRIGATE_H, NPChar), "ship7");				
				if (iTest_ship == 3 && sti(NPChar.nation) == SPAIN) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_GALEON_H, NPChar), "ship7");
				if (iTest_ship == 3 && sti(NPChar.nation) == ENGLAND) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_FRIGATE_H, NPChar), "ship7");
				if (iTest_ship == 3 && sti(NPChar.nation) == HOLLAND) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_EASTINDIAMAN, NPChar), "ship7");	
			}
		}
		...
		//Восьмой слот
		{
			...
			{
				iTest_ship = rand(5);
				if (iTest_ship == 0) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_NAVIO, NPChar), "ship8");
				if (iTest_ship == 1) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_FRIGATE_H, NPChar), "ship8");					
				if (iTest_ship == 2) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_FRIGATE, NPChar), "ship8");					
				if (iTest_ship == 3) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_GALEON_H, NPChar), "ship8");
			}
		}
		...
		if (sti(PChar.rank) > 8)
		{
			...
			if (iTest_ship == 8 && sti(NPChar.nation) == FRANCE) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_POLACRE, NPChar), "ship14");
			...
			if (iTest_ship == 8 && sti(NPChar.nation) == FRANCE) FillShipParamShipyard(NPChar, GenerateStoreShipExt(SHIP_POLACRE, NPChar), "ship15");
		}
...
*/	
	int price = makeint( sti(RealShips[sti(chref.Ship.Type)].Price) * (10 + MOD_SKILL_ENEMY_RATE) * (3 + GetNationRelation2MainCharacter(sti(NPChar.nation)))/360.0 );
