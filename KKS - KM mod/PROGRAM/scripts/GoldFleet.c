...
void GoldFleet()
{
	...
	if (!CheckAttribute(pchar, "questTemp.MapBest")) { // mitrokosta отличная карта
		sName = "Сантисима Тринидад";
	}
	chref = GetCharacter(NPC_GenerateCharacter("Head_of_Gold_Squadron", "off_spa_2", "man", "man", 5, SPAIN, 31, true, "governor"));
	FantomMakeCoolSailor(chref, SHIP_LSHIP_QUEST, sName, CANNON_TYPE_CANNON_LBS36, 105, 105, 105);
	FantomMakeCoolFighter(chref, 45, 100, 100, "blade_18", "pistol9", "bullet", 100); // 170912
	...
	LAi_SetHP(chref, 300 + makeint(pchar.rank) * 2, 300 + makeint(pchar.rank) * 2);
	TakeNItems(chref, "potion2", 8);
	TakeNItems(chref, "potion3", 5); 
	TakeNItems(chref, "cirass4", 1); 
	SetCharacterPerk(chref, "Tireless");
	SetCharacterPerk(chref, "HardHitter");
	SetCharacterPerk(chref, "AdvancedDefense");
	SetCharacterPerk(chref, "SwordplayProfessional");
	SetCharacterPerk(chref, "MusketsShoot");
	SetCharacterPerk(chref, "LongRangeShoot");
	SetCharacterPerk(chref, "FastReload");
	SetCharacterPerk(chref, "HullDamageUp");
	SetCharacterPerk(chref, "SailsDamageUp");
	SetCharacterPerk(chref, "CrewDamageUp");
	SetCharacterPerk(chref, "CannonProfessional");
	SetCharacterPerk(chref, "GrapplingProfessional");
	SetCharacterPerk(chref, "BasicBattleState");
	SetCharacterPerk(chref, "AdvancedBattleState");
	SetCharacterPerk(chref, "ShipDefenseProfessional");
	SetCharacterPerk(chref, "ShipSpeedUp");
	SetCharacterPerk(chref, "ShipTurnRateUp");
	SetCharacterPerk(chref, "SailingProfessional");
    ...
    for(int k = 1; k <= GoldCapNum ; k++)
	{
		...
		if(k == capHasGold)
		{
            ...
            iSpace = iSpace / 4;
			Fantom_SetCharacterGoods(sld, GOOD_GOLD, (3*iSpace), 1);
