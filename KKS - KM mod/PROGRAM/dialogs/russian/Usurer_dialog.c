	...
	switch(Dialog.CurrentNode)
	{
		...
		//<-- Бремя гасконца
			...
			if(NPChar.quest.meeting == "0")
			{
				...
				link.l1 = RandPhraseSimple(LinkRandPhrase("Я тоже рад приветствовать вас. Вот, прогуливался по городу и решил заглянуть к вам.","Очень приятно, я "+GetFullName(pchar)+", будем знакомы.",""+GetFullName(pchar)+", капитан корабля "+pchar.ship.name+", рад знакомству. А у вас тут, как я понимаю, банк?"), RandPhraseSimple("Будем знакомы, я "+GetFullName(pchar)+". капитан корабля "+pchar.ship.name+".  Я вот решил заглянуть, познакомиться.","Ага, значит я зашёл в местный банк? Рад знакомству, я "+GetFullName(pchar)+", капитан собственного корабля."));
				link.l1.go = "next";
			...
			else
			{
				...
				link.l1 = pcharrepphrase(RandPhraseSimple(LinkRandPhrase("Шутки в сторону, ростовщик, я пришёл по делу.","Много болтаешь, дружок. Давай-ка перейдём к делам.","Деньги не пахнут, слышал такую умную фразу? Ну-ка, зажимай нос и переходи к делу."), LinkRandPhrase("Это так здесь встречают клиентов? Твоё счастье, что я сегодня в добром настроении.","Ха! Вижу, здесь мне не рады? Ну, так я и не золотой дублон, чтобы тебе нравиться. И вообще, давай-ка к делу, меняла.","Эх, и поучил бы я тебя вежливости, дружок... да некогда мне. Займёмся, пожалуй, мы с тобой денежными вопросами.")),
				RandPhraseSimple(LinkRandPhrase("Рад видеть старого знакомого!","К вам всегда приятно зайти, дружище. Ну что, перейдём к делам?","Очень рад, очень рад. Обсудим дела, дружище?"),LinkRandPhrase("Весьма рад новой встрече. Ну, что нового в финансовых морях-океанах?","Рад, что помните меня, любезный. Я тороплюсь, так что, может быть, сразу перейдём к делам?","Привет, старина! Рад снова видеть старых знакомых!")));
				link.l1.go = "next";
			}
			break;
...
*/			
		...
		case "small":
			Pchar.Quest.Loans.(NPC_Area).Sum = 10000 + 1000*makeint(Pchar.rank);
			...
			Pchar.Quest.Loans.(NPC_Area).Interest = 33 - makeint((GetSummonSkillFromName(pchar, "Commerce")+GetSummonSkillFromName(pchar, "Leadership"))/10);
		break;

		case "Medium":
			Pchar.Quest.Loans.(NPC_Area).Sum = 25000 + 2500*makeint(Pchar.rank);
			...
			Link.l3 = "Думаю, мне лучше не влезать в долги. До свидания.";
			Link.l3.go = "ExitDelLoan1";
			Pchar.Quest.Loans.(NPC_Area).Interest = 20 - makeint((GetSummonSkillFromName(pchar, "Commerce")+GetSummonSkillFromName(pchar, "Leadership"))/20);
		break;

		case "Large":
			Pchar.Quest.Loans.(NPC_Area).Sum = 50000 + 10000*makeint(Pchar.rank);
			...
			Link.l3 = "Думаю, мне лучше не влезать в долги. До свидания.";
			Link.l3.go = "ExitDelLoan1";
			Pchar.Quest.Loans.(NPC_Area).Interest = 15 - makeint((GetSummonSkillFromName(pchar, "Commerce")+GetSummonSkillFromName(pchar, "Leadership"))/20);
		break;
		...
		case "Period":
			Pchar.Quest.Loans.(NPC_Area).Period = makeint(makeint(Pchar.reputation.nobility)/10) + 3;			
		...
		case "GiveTaskSlave":
			...
			pchar.questTemp.Slavetrader.iSlaveMoney = sti(pchar.questTemp.Slavetrader.iSlaveQuantity) * 500;
		...
		case "GiveTaskSlave_1":
			dialog.Text = "Хорошо, слушайте. Сейчас нет на всех Карибах прибыльнее дела, чем работорговля. Я не плантатор, но у меня есть надёжный покупатель на этот товар, и не один\nВ общем, мне нужны рабы в количестве " + pchar.questTemp.Slavetrader.iSlaveQuantity + " штук. За каждого я готов заплатить по 500 песо - это неплохая цена, согласитесь. Всего сумма сделки составит " + FindRussianMoneyString(sti(pchar.questTemp.Slavetrader.iSlaveMoney)) + " золотом.";
		...
		case "FindRat_6":
			...
			link.l1 = "Хм... Этого негодяя следует вздернуть на ноке рея! Сообщите мне более подробную информацию о нём, ну и, конечно, размер гонорара за работу.";
		...
		case "Havana_fort":
			dialog.text = "Вы очень вовремя, " + pchar.name + "! Я уже было начал волноваться, и собирался послать на ваши поиски человека\nНу-с, приступим! Задача на этот раз перед вами будет стоять очень серьёзная, но и вознаграждение будет соответствующим. После проведенной операции вы получите миллион, или больше.";
