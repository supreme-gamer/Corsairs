			...
			else
			{
                ...
                {//->
                    ...
                    else
                    {
						...
						{
							link.l15 = UpperFirst(GetAddress_FormToNPC(NPChar)) + ", я обнаружил банду контрабандистов, чьего главаря недавно поймала стража. Они планируют вызволить его. Их корабль '" + PChar.GenQuest.JusticeOnSale.ShipName + "' стоит у " + XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId + "Gen") + "."; // belamour gen
		...
		case "node_1":
            ...
            {//->
                ...
                else
                {
					...
					if(CheckAttribute(PChar, "GenQuest.JusticeOnSale.MayorWait") && CheckAttribute(PChar, "GenQuest.JusticeOnSale.CityId") && NPChar.location == PChar.GenQuest.JusticeOnSale.CityId + "_townhall")
					{
						link.l15 = "Милорд, я обнаружил банду контрабандистов, чьего главаря недавно поймала стража. Они планируют вызволить его. Их корабль '" + PChar.GenQuest.JusticeOnSale.ShipName + "' стоит у " + XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId + "Gen") + "."; // belamour gen
						link.l15.go = "JusticeOnSale_1";
					}
					//<-- Правосудие на продажу
...
//navy <--
			if (!CheckAttribute(npchar, "work_date") || GetNpcQuestPastDayParam(npchar, "work_date") > 2 || bBettaTestMode || CheckAttribute(pchar, "CSM.FreeGiveQuest")) // krizis
    		{	
...
//<-- осады homo
		...
		case "Sharlie_2":
			dialog.text = "И всё же я хотел бы загладить свою вину перед вами. Возьмите это снаряжение и карту Мартиники от меня в подарок. А еще я компенсирую ваши финансовые потери.";
		...
		case "Sharlie_3":
			...
			TakeNItems(pchar, "map_martiniqua", 1); 
