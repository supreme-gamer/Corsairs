...
//navy <--
			...
			{
				if (NPChar.id == "LaVega_trader" || NPChar.id == "LeFransua_trader" || NPChar.id == "Pirates_trader" || NPChar.id == "PuertoPrincipe_trader")
				{
		...
		case "storage_01a": // krizis
			NPChar.MoneyForStorage = GetStoragePriceExt(NPChar, pchar); 
			dialog.text = "Да, как и тогда: припортовый пакгауз - вместимость 50000 ц. За " + FindRussianMoneyString(sti(NPChar.MoneyForStorage)) + " в месяц я готов обеспечить сохранность ваших запасов. "+
				"Это включает услуги по охране, защиту от подтопления и борьбу с грызунами. Ну, что скажете? И да... оплату за месяц попрошу вперёд.";	
			link.l1 = "Годится. Вот ваши монеты.";
		...
		case "storage_4":
			...
			{
				dialog.text = "Забирайте свой товар и я закрою склад. Возвращайтесь, если передумаете!"; // krizis
				link.l1 = "Хорошо.";
...
//Jason --> ----------------------------------мини-квест Бесчестный конкурент------------------------------------
			if (!CheckAttribute(pchar, "questTemp.Shadowtrader") && sti(npchar.nation) != PIRATE && sti(pchar.rank) < 13 && drand(2) == 0) 
			{
...
//navy <--
            ...
            if (sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)
			{
				...
				if(CheckAttribute(NPChar,"Storage") && Pchar.location.from_sea == "Pirates_town")
				{
					...
					else
					{
						...
						{
							link.l7 = "Помнится, у вас тут пакгауз пустует. Не занял ли его кто? Сдал его вам обратно, да вот опять потребовался.";
		...
		case "generate_quest":
			if (npchar.quest.trade_date != lastspeak_date || bBettaTestMode || CheckAttribute(pchar, "CSM.FreeGiveQuest")) // krizis
			{
