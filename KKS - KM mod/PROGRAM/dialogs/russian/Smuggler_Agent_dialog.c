void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Meeting_3":
			...
			// Чтобы было 2 корабля
			// Ранг корабля меньше первого
			if (!csmCA(pchar, "CSM.FreeSmuggling")) // --> CSM
			{
				for (i=0; i<COMPANION_MAX; i++)
				{
					...
					if (iChIdx >= 0)
					{
						...
						shipClass = GetCharacterShipClass(sld);
						if(shipClass == 1)
						{			
							iTmp = true;
							break;
			...
			*/
			...
			if (iTmp)
			{
				dialog.text = NPCStringReactionRepeat("Ты бы еще на королевском мановаре явил"+ GetSexPhrase("ся","ась") +". Да твою посудину за милю видать из форта. Мы не будем рисковать своими головами. Приходи на корабле ниже первого ранга, и чтоб в эскадре их было не больше двух.", 
					"Я что, непонятно выражаюсь? Найди себе суденышко поменьше, тогда и приходи.", 
					"Ты что, тупица или притворяешься? Говорю же тебе - найди себе фрегат, ну галеон тяжелый на крайний случай, или сделка не состоится.",
