...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "PegYou":
			...
			{
				if (GetCharacterItem(pchar, "gold_dublon") >= 10) //krizis
				{
					link.l2 = RandPhraseSimple("Я думаю мы сможем договориться? (10 дублонов)", "Я "+ GetSexPhrase("слышал","слышала") +", солдатов обделяют в жаловании... Как насчет небольшой надбавки?(10 дублонов)");
					link.l2.go = "Bribe_dublon";
				}
				if (sti(Pchar.money) >= 2500) //krizis
				{
					link.l3 = RandPhraseSimple("Я думаю мы сможем договориться? (2500 песо)", "Я "+ GetSexPhrase("слышал","слышала") +", солдатов обделяют в жаловании... Как насчет небольшой надбавки?(2500 песо)");
					link.l3.go = "Bribe_peso";
				}
			}
			
		break;

		case "Bribe_dublon": //krizis
			TakeNItems(pchar, "gold_dublon", -10);
			dialog.text = RandPhraseSimple("Кхм... Да! Пожалуй, что так... Эмм... Тут всё впорядке!.. Не задерживайтесь.", "Хм, сейчас я буду смотреть в другую сторону, но учти, я все равно слежу за тобой и не забывай, что нас тут целый гарнизон!");
			link.l1 = RandPhraseSimple("Благодарю!", GetSexPhrase("Рад","Рада")+", что мы поняли друг друга...");
			link.l1.go = "exit";
			if (sti(pchar.questTemp.stels.landSolder) != GetDataDay())
			{
		...
		case "LicenceOk":
			...
			if (iTemp == -1)
			{
				...
				link.l1 = RandPhraseSimple("Как бы не так!", "После дождичка, в четверг...");
				link.l1.go = "fight";
				if (csmCA(pchar, "CSM.Bribe") && sti(Pchar.money) >= 10000) //krizis
				{
					link.l2 = "Я полагаю с этим можно что-то сделать? Это недоразумение!";
					link.l2.go = "BribeLicence"
				}
				TakeNationLicence(HOLLAND);
				break;
			}
		...
		case "BribeLicence": //krizis			
			dialog.text = RandPhraseSimple("Хорошо, не переживай ты так! 10 000 песо штрафа и гуляй, но лицензия уже просрочена и подлежит изъятию!", "Ох. За что же мне всё это... Ухх... Заплатите 10 000 песо штрафа в казну колонии и можете быть свободны. Но лицензия просрочена и ни на что не годна!");
			link.l1 = RandPhraseSimple("Хорошо, я выплачиваю компенсацию.", "Главное, чтобы не было проблем. У меня еще дела в городе.");
			link.l1.go = "exit";
			AddMoneyToCharacter(pchar, -10000)
			if (GetCharacterItem(pchar, "HolTradeLicence") >= 1) TakeNationLicence(HOLLAND); // на всякий пожарный, может и не пригодится... krizis
		break;
