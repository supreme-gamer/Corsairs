...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "hired":
			...
			if (pchar.location == Get_My_Cabin() && !CheckAttribute(NPChar, "quest.off_sluzba")) // ещё не служит .  служба
			{			
			 Link.l541 = "Как насчёт того, чтобы пойти ко мне на постоянную службу?";
               Link.l541.go = "post_sluzba"; // переход на кейса диалога найма. 
			}
...
/////////////////////    служба оффа //////////////////////////////////////////////////////////////////////////////
		case "post_sluzba":  // служба
            int iRank = sti(NPChar.rank)*2500;
            int offcost = (MOD_SKILL_ENEMY_RATE*5)*1000;
            int sumCost = (iRank + offcost);
            PChar.sumcost.Money = sumCost;
            dialog.text = "С большим удовольствием буду служить вам на постоянной основе, капитан, за "+(sumCost)+" песо авансом!";
            link.l1 = "Вот и отлично, я согласен!";
            link.l1.go = "post_sluzba2";
        break;
        case "post_sluzba2":
            if (sti(Pchar.money) >= sti(PChar.sumcost.Money))
           {
               dialog.text = "Спасибо, кэп! Я оправдаю ваше доверие!";
               link.l1 = "Я в этом не сомневаюсь!";
               link.l1.go = "post_sluzba3"; // post_sluzba3
           }
           else{
              dialog.text = "Капитан, вот когда у вас будут деньги, тогда и поговорим.";
              link.l1 = "Действительно, чего это я. Как пополню карманы, мы продолжим этот разговор...";
              link.l1.go ="exit";
            }
        break;
        case "post_sluzba3":
            DialogExit();
            AddMoneyToCharacter(Pchar, -sti(PChar.sumcost.Money));
            Npchar.loyality = MAX_LOYALITY;
            npchar.OfficerWantToGo.DontGo = true; // не уходит
            npchar.quest.off_sluzba = true; // уже на службе. 
		break;
/////////////////////////          <===  служба ==== /////////////////////////////////////////////////////////		
	...
	case "WantToGo_Stay_2":
		dialog.text = LinkRandPhrase("Ну, это меняет дело. "+sti(NPChar.rank)*300+" песо на руки и я остаюсь.", "Ну, пожалуй за "+sti(NPChar.rank)*300+" песо я бы остался.", "Хм, раз пошёл такой разговор, то за "+sti(NPChar.rank)*300+" песо можно и послужить ещё.");
		if (sti(Pchar.money) >= sti(NPChar.rank)*300)
		{
