...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    switch (Dialog.CurrentNode)
	{
		...
		case "Tonzag_check"://начинаем проверять нашего ГГ по всем статьям
			...
			if(GetSummonSkillFromName(pchar, SKILL_F_LIGHT) < 35 || GetSummonSkillFromName(pchar, SKILL_FENCING) < 35 || GetSummonSkillFromName(pchar, SKILL_F_HEAVY) < 35 || GetSummonSkillFromName(pchar, SKILL_PISTOL) < 35)//слабое фехтование
			{
			...
			if(sti(Pchar.rank) > 25)//высокий ранг
			{
