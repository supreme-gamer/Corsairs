...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "bakaut_done_1":
			dialog.text = "Я стараюсь не накапливать больших партий. Та, что была украдена Моллиганом - исключение. Властям не понравится, если они прознают, что бакаут уходит на сторону. Поэтому давай договоримся так: к каждому четырнадцатому и двадцать восьмому числу месяца я буду собирать на складе по 50 штук бакаута\nЕсли ты хочешь его купить - приноси сразу полторы тысячи дублонов и забирай всю партию. Если ты не придёшь в указанный день - я продаю её другому покупателю. Таким образом, ты сможешь в месяц забирать у меня по 100 штук бакаута. Замётано?";
	...
	//-----------------------------------генератор торговли бакаутом--------------------------------------------
		case "trade_bakaut":
			dialog.text = "Хорошо, я не возражаю. У меня на складе есть пятьдесят единиц. Как ты помнишь, цена - по тридцать дублонов за штуку.";
			if (GetCharacterItem(pchar, "gold_dublon") >= 1500)
			{
				link.l1 = "Всё по уговору. Вот, держи свои полторы тысячи дублонов.";
		...
		case "trade_bakaut_1":
			RemoveItems(pchar, "gold_dublon", 1500);
			Log_Info("Вы отдали 1500 дублонов");
			PlaySound("interface\important_item.wav");
		...
		case "trade_bakaut_2":
			...
			link.l1.go = "exit";
			AddCharacterGoods(pchar, GOOD_SANDAL, 50);
