	...
	switch(Dialog.CurrentNode)
	{
		...
		case "HWIC_officer":
			...
			if (CheckAttribute(pchar, "questTemp.HWIC.CanTake") && !CheckAttribute(pchar, "questTemp.HWIC.CanTake.Holl") && !CheckAttribute(pchar, "questTemp.HWIC.Eng") && !CheckAttribute(pchar, "questTemp.HWIC.Self"))
			{
				...
				link.l1 = "Я зашёл по торговым делам. Лицензии, корабельные приборы и прочее...";
			...
			if (CheckAttribute(pchar, "questTemp.HWIC.Eng"))
			{
				...
				link.l2 = "Я зашёл по торговым делам. Лицензии, корабельные приборы и прочее...";
		...
		case "HWIC_headclerk":
			...
			if (ChangeCharacterHunterScore(Pchar, "holhunter", 0) > 12)
			{
				dialog.text = "Гм. Мне кажется, что не всё тут в порядке... Точно! Вы объявлены в розыск за многочисленные преступления против Голландии! Солдаты, хватайте его!";
				link.l1 = "А-ать!";
				link.l1.go = "fight";
				break;
			}
		...
		case "silk_info_1":
			dialog.text = "Поставки у меня идут регулярно, но в небольших объёмах. Для вас я могу оставлять по сто рулонов товара каждые две недели. Стоимость одного рулона - двадцать золотых дублонов. Давайте поступим так: каждого первого и пятнадцатого числа месяца я буду придерживать для вас сто рулонов. Если вы не придёте за ними в эти дни - я продаю другому покупателю.";
			link.l1 = "То есть никаких обязательств я не несу?";
			link.l1.go = "silk_info_2";
		break;
		
		case "silk_info_2":
			dialog.text = "Именно! Хотите - покупаете, хотите - нет. Всю партию оплачиваете полностью - две тысячи дублонов. Надеюсь, вы станете моим постоянным покупателем.";
		...
		case "trade_silk":
			...
			if (GetCharacterItem(pchar, "gold_dublon") >= 2000)
			{
				link.l1 = "Конечно! Вот, возьмите две тысячи дублонов.";
		...
		case "trade_silk_1":
			RemoveItems(pchar, "gold_dublon", 2000);
			Log_Info("Вы отдали 2000 дублонов");
