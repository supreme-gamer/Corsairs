...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Sergio_28":
			dialog.text = "В течение месяца я буду откладывать понемногу смолы для вас и сформирую партию в 120 бочек. Я прикинул - именно столько я смогу собрать за месяц, не возбудив подозрения властей. Давайте договоримся, что каждого пятого числа месяца я буду наведываться сюда, на маяк, и если вы решите приобрести смолы - то пожалуйста. Стоимость одной бочки смол - десять золотых дублонов\nДеньги за всю партию оплачиваете сразу, сразу всю и забираете. Если вы не пришли пятого числа - то я начинаю распродавать товар другим покупателям, либо сдаю губернатору. То есть, ничто вас не обязывает, сугубо ваше личное желание. По рукам?";
		...
		case "oil_trade_1":
			dialog.text = "Да. Тысяча двести дублонов при вас?";
			if (GetCharacterItem(pchar, "gold_dublon") >= 1200)
			{
		...
		case "oil_trade_2":
			chrDisableReloadToLocation = true;//закрыть локацию
			RemoveItems(pchar, "gold_dublon", 1200);
			Log_Info("Вы отдали 1200 дублонов");
			PlaySound("interface\important_item.wav");
		...
		case "oil_trade_4":
			...
			LAi_SetActorType(npchar);
			LAi_ActorGoToLocation(npchar, "reload", "reload1", "none", "", "", "OpenTheDoors", 5);
			AddCharacterGoods(pchar, GOOD_OIL, 120);
