void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Escort_choice"://первые развилки - выбираем между городом, бухтой и необитайкой
			...
			switch (drand(2))
			{
				case 0://город дружественный
					...
					pchar.GenQuest.Escort.Trader.Money = makeint(sti(pchar.GenQuest.Escort.Trader.DaysQty)*1000*ShipFactorMC()+sti(pchar.GenQuest.Escort.Trader.ShipType)*1200);//оплата от расстояния и классов кораблей ГГ и неписи
					...
					link.l1 = "Ну что же, почему бы и нет? Вдвоём всегда безопаснее, это понятно. Я согласен.";
				...
				case 1://бухта
					...
					pchar.GenQuest.Escort.Trader.Money = makeint(sti(pchar.GenQuest.Escort.Trader.DaysQty)*1000*ShipFactorMC()+sti(pchar.GenQuest.Escort.Trader.ShipType)*1200);//оплата
		...
		case "EscortTrader_complete_5":
			...
			link.l1 = "Я сказал - человек человеку - волк, да? Вы так легко и беззастенчиво 'сдаёте' своего коллегу... Впрочем, мне до вашей совести дела нет, а за наводку спасибо. Счастливо оставаться, сударь!";
		...
		case "Race_begin_2":
			dialog.text = "Ну, не все такие, дружище. Вот я, например, всегда в одиночку хожу - мне лишний балласт не нужен. Я отлично знаю воды Карибского моря, и, позволю себе заметить, неплохо управляюсь со своим кораблём и командой. Так что я запросто уйду в море от любого пирата.";
