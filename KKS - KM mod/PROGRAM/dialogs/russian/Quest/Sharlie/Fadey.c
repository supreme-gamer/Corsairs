...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "persian_4":
			dialog.text = "Водочка - это дело дюже хорошее, но моя родная сторона славится не только одной ею. Есть один товар, который делают только у нас, в России, и который я привожу сюда для негоции. Сей товар интересен любому капитану, ибо являет собой часть корабля. Я говорю о пеньковых канатах\nЭти канаты делаются из волокон конопляных, на прядильнях, руками умелых мастеровых. Они не боятся ни влаги, ни соли морской, и посему имеют таки огромный спрос у корабельных дел мастеров. Как говорят - лучшего материала для оснастки нет на свете\nЕжемесячно купеческие суда доставляют мне сюда небольшую партию канатов этих. Обычно к середине каждого месяца мой склад пополняется на сотню-полторы бухт. Так вот, я предлагаю вам, "+pchar.name+", покупать их у меня в количестве сто бухт за один присест. Стоимость одной бухты - десять золотых дублонов\nУсловий у меня всего два: я придерживаю канаты ваши до двадцатого числа каждого месяца - раз, вы покупаете всю партию в сотню бухт целиком - два. Вы мне тысячу золотых - я вам канаты. Скажу вам по секрету, "+pchar.name+": говорят, есть места, где можно перепродать сей товар дороже, но вот беда - я не ведаю, где и кому\nОднако, меня и нонешнее положение дел устраивает полностью. А ежели вы отыщете доброго купца - то сможете иметь выгоду немалую.";
			link.l1 = "Ясно. Отличное предложение, Фадей! Думаю, буду пользоваться предоставленной возможностью. Итак, каждого двадцатого числа месяца тысяча дублонов за сотню бухт?";
		...
		// торговля канатами
		case "ropes":
			dialog.text = "Пожалуйте, друг мой любезный, для вас всё готово! Сто бухт по десять дублонов за бухту.";
			if (GetCharacterItem(pchar, "gold_dublon") >= 1000)
			{
				link.l1 = "Все по уговору. Вот, возьмите тысячу дублонов.";
				link.l1.go = "ropes_1";
			}
			else
			{
		...
		case "ropes_1":
			...
			Log_Info("Вы отдали 1000 дублонов");
			PlaySound("interface\important_item.wav");
