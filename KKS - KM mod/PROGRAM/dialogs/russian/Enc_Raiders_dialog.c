void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "exit_Robbed":
			int iMoney = makeint(PChar.rank)*1000;
		...
		case "Node_2":
			...
			Link.l1 = "Проклятье! Мерзавец! У меня с собой только " + makeint(PChar.rank)*1000 + " песо.";
		...
		case "CheckMoney":
			if(makeint(PChar.rank)*1000 >= makeint(Pchar.rank)*100)
			{
