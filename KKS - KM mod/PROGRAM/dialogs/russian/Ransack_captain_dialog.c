...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		if (CheckAttribute(pchar, "questTemp.FMQG") && pchar.questTemp.FMQG != "fail" && CheckAttribute(pchar, "questTemp.FMQG.Prisoner") && GetFullName(npchar) == pchar.questTemp.FMQG.Prisoner)
		{
			...
			link.l99 = "Я знаю.";
			link.l99.go = "exit";
			link.l100 = "В бою ты показал себя талантливым и рассудительным командиром. А под моим началом ты сможешь заработать гораздо больше чем, на государевой службе. Пойдёшь в мою команду офицером?";
			link.l100.go = "free_to_officer";
