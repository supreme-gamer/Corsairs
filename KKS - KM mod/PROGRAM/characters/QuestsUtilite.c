...
void FillAboardCabinBox(ref _location, ref _npchar)
{
    ...
    if (ok) // не квестовый
    {
		...
		if(iNation == PIRATE)
		{
			...
			if (nLuck > rand(100) && GetCharacterItem(pchar, "map_full") == 0)  // шанс 1/30 
			{
		...
		if (CheckAttribute(_npchar, "Ship.Mode") && _npchar.Ship.Mode == "Trade")  // торговец
		{
			...
			_location.box1.money = pow(31-iTemp, 3) * (1 + 0.06 * (rand(10 - iTemp)) + 0.06 * GetCharacterSkillToOld(Pchar, "commerce")) / ((rand(12)+100) * 0.01);
				
			if(rand(5) > 2)
			{
				_location.box1.items.gold_dublon = rand(100) + 50;
				if(drand(20) > 17) _location.box1.items.rat_poison = 3;			
			}	
		}
		else // все остальные
		{
			if(rand(3) == 1) _location.box1.items.gunpowder = 5 + rand(10);
			_location.box1.money = pow(31-iTemp, 3) * (0.7 + 0.06 * (rand(10 - iTemp)) + 0.06 * GetCharacterSkillToOld(Pchar, "commerce")) / ((rand(12)+100) * 0.01);
			if(rand(4) == 1)
			{
				_location.box1.items.gold_dublon = rand(25) + 25;
		...
		{
			if (drand(3) == 1 && CheckAttribute(pchar, "questTemp.AdmiralMap")) // адм.карты
			{
				amap = SelectAdmiralMaps();
				if (amap != "") _location.box1.items.(amap)	= 1;
			}
...
string GetStrSmallRegister(string sBase)
{
	...
	for (int i=0; i<qty; i++)
	{
		...
		switch (Simbol)
		{
			...
			case "Ъ": sResult += "ъ"; continue; break;
			case "Ы": sResult += "ы"; continue; break;
			case "Ь": sResult += "ь"; continue; break;
...
String ToUpper(String _text)
{
	...
	for(int i = 0; i < length; i++)
	{
		...
		switch(symbol)
		{
			...
			case "ъ": retString += "Ъ"; continue; break;
			case "ы": retString += "Ы"; continue; break;
			case "ь": retString += "Ь"; continue; break;
...
{
	if (csmCA(pchar, "CSM.LSCFreeArrival")) // --> CSM
		return true;
...
*/
	int price = makeint( 100000 * (5 + MOD_SKILL_ENEMY_RATE) * (3 + GetNationRelation2MainCharacter(sti(NPChar.nation)))/360.0 );
...
{
	if (csmCA(pchar, "CSM.NotDeleteShipsInStock")) // --> CSM
		return;
...
{
	if (csmCA(pchar, "CSM.NoCheckAndIdentifyShips")) // --> CSM
		return false;
