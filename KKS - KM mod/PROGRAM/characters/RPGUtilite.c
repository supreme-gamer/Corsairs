...
{
	if (csmCA(_refCharacter, "CSM.EnergyBonus")) // --> CSM
		return 999.0;
	...
	// Sticks --> apply energy mod if exist
	if (CheckAttribute(_refCharacter, "CSM.EnergyMod"))
		ret += stf(_refCharacter.CSM.EnergyMod);
	// <-- Sticks
...
int ApplySPECIALCirassPenalty(ref rChar, String sSkillName)
{
	...
	// --[ CSM
	if (iValue < 0 && csmCA(rChar, "CSM.CuirassNoPenalty"))
		iValue = 0;
	// CSM ]--
		...
		if (CheckCharacterPerk(_chref, "SharedExperience"))
		{
			for(i=0; i<iPas; i++)
			{
				...
				if(cn!=-1)
				{
				   ...
				   {
				       AddCharacterExpToSkill(GetCharacter(cn), _skill, _addValue / 1.4);
				   }
				   else
				   {
...
{
	if (CheckAttribute(_chref, "Skill.FencingS") && !csmCA(_chref, "CSM.CargoTank")) // --> CSM
    {
        int iBonus = 0;
        if (IsCharacterPerkOn(_chref, "Grus")) iBonus = 50;
...
void CCS_SetNewMainCharacter(ref ch, int num)
{
	...
	switch (num)
	{
		...
		//	ch.lastname				= "Брут";
		//	ch.nameGen				= "Иеремии";
		//	ch.lastnameGen			= "Брута";
		//	ch.nameDat				= "Иеремие";
		//	ch.lastnameDat			= "Бруту";
		//	ch.sex					= "man";
		//	ch.model.animation 		= "man_A";
		//	ch.HeroParam.HeroType 	= "HeroType_1";
		//	ch.nation				= ENGLAND;
		//	ch.BaseNation			= ENGLAND;
		//	ch.info					= "Торговец черным золотом и контрабандист. Жесток, коварен и хитер. Ловил рабов для англичан на побережье Африки." + 
		//							  " После того как крупно поругался с марокканскими пиратами и они назначили награду за его голову - бежал в Новый Свет, где мечтает основать собственную кровавую торговую империю.";									
		...
		case 2: // Диего  де Монтойя
			...
			ch.FaceId 				= 289;
			ch.HeroModel 			= "Port_0,Port_2,Port_3,Port_4,Port_5,protocusto,Port_8,Port_1";
			ch.name 				= "Клаус";
			ch.lastname				= "Штертебекер";
			ch.nameGen				= "Клауса";
			ch.lastnameGen			= "Штертебекера";
			ch.nameDat				= "Клаусу";
			ch.lastnameDat			= "Штертебекеру";
			ch.sex					= "man";
			ch.model.animation 		= "man_A";
			ch.HeroParam.HeroType 	= "HeroType_1";
			ch.nation				= ENGLAND;
			ch.BaseNation			= ENGLAND;
			ch.info					= "Торговец черным золотом и контрабандист. Жесток, коварен и хитер. Скупал рабов для англичан на побережье Африки." + 
									  " После того как крупно поругался с марокканскими пиратами и они назначили награду за его голову - бежал в Новый Свет, где мечтает основать собственную кровавую торговую империю.";									
			
		break;	
		
		case 3: // Вильям Паттерсон
			
			ch.FaceId 				= 202;
			ch.HeroModel 			= "Pat_0,Pat_2,Pat_3,Pat_4,Pat_5,protocusto,Pat_9,Pat_1";
			ch.name 				= "Вильям";
		...
		case 4: //	Эспиноза в шляпе;
			
			ch.FaceId 				= 728;
			ch.HeroModel 			= "Espinosa_alt,Espinosa_alt_Cirass1,Espinosa_alt_Cirass2,Espinosa_alt_Cirass3,Espinosa_alt_Cirass4,protocusto,Espinosa_alt_hat,Espinosa_alt_hat";
			ch.name 				= "Диего";
		...
		case 5: //			Миешль де Граммон;
			
			ch.FaceId 				= 415;
			ch.HeroModel 			= "Resc_0,Resc_2,Resc_3,Resc_4,Resc_5,protocusto,Resc_9,Resc_7";
			ch.name 				= "Мишель";
			ch.lastname				= "де Граммон";
			ch.nameGen				= "Мишеля";
			ch.lastnameGen			= "де Граммона";
			ch.nameDat				= "Мишелю";
			ch.lastnameDat			= "де Граммону";
			ch.sex					= "man";
			ch.model.animation 		= "man_A";
			ch.HeroParam.HeroType 	= "HeroType_4";
			ch.nation				= FRANCE;
			ch.BaseNation			= FRANCE;
			ch.info 				= "Внебрачный сын французского дворянина, в Старом Свете был поверенным в дела Мальтийского ордена. Но неуёмная жажда приключений вышвырнула его из благополучной Европы в джунгли Карибского архипелага." +
									  " Что он здесь ищет? Лишь судьба знает ответ на этот вопрос...";
		...
		case 6: // Питер Блад
			...
			ch.FaceId 				= 421;
			ch.HeroModel 			= "blad_0,blad_2,blad_3,blad_4,blad_5,protocusto,blad_9,blad_1";
			ch.name 				= "Питер";
			ch.lastname				= "Блад";
			ch.nameGen				= "Питера";
			ch.lastnameGen			= "Блада";
			ch.nameDat				= "Питеру";
			ch.lastnameDat			= "Бладу";
			ch.sex					= "man";
			ch.model.animation 		= "man_A";
			ch.HeroParam.HeroType 	= "HeroType_3";
			ch.nation				= ENGLAND;
			ch.BaseNation			= ENGLAND;
			ch.info 				= "В Старом Свете он был успешным врачом, но обстоятельства сделали его рабом на плантациях Барбадоса." +
									  " Теперь ему необходимо вернуть свой социальный статус и восстановить то положение в обществе, которого заслуживают его амбиции, темперамент и интеллект."; 
				  
		break;
		
		case 7: //	Эспиноза дворянин;
			
			ch.FaceId 				= 702;
			ch.HeroModel 			= "Espinosa_0,Espinosa_2,Espinosa_3,Espinosa_4,Espinosa_5,protocusto,Espinosa_8,Espinosa_9";
			ch.name 				= "Арман";
			ch.lastname				= "Эспиноза";
			ch.nameGen				= "Армана";
			ch.lastnameGen			= "Эспинозу";
			...
			ch.nation				= SPAIN;
			ch.BaseNation			= SPAIN;
			ch.info 				= "Знаменитый испанский гранд, достигший всего в Старом Свете. Впечатленный успехами своего брата Диего на Карибах, решил тоже заняться каперством и торговлей в колониях." + 
			                          " Теперь его цель - превзойти брата, заодно упрочив положение дворянского рода Эспиноза в мире...";
									  
		break;
		
		case 8: //	Алекс Эксквемелин;
			
			ch.FaceId 				= 716;
			ch.HeroModel 			= "Blood5,Blood5_Cirass,Blood5_Cirass1,Blood5_Cirass2,Blood5_Cirass3,protocusto,Blood5,Blood5";
			ch.name 				= "Алекс";
			ch.lastname				= "Эксквемелин";
			ch.nameGen				= "Алекса";
			ch.lastnameGen			= "Эксквемелина";
			ch.nameDat				= "Алексу";
			ch.lastnameDat			= "Эксквемелину";
			ch.sex					= "man";
			ch.model.animation 		= "man_A";
			ch.HeroParam.HeroType 	= "HeroType_4";
			ch.nation				= HOLLAND;
			ch.BaseNation			= HOLLAND;
			ch.info 				= "Голландский авантюрист и торговец. Прибыл в Новый Свет в поисках приключений и прибыли. Рассчитывает быстро обогатиться и сделать себе имя." + 
			                          " Несмотря на спокойный нрав и миролюбивую внешность - отличный рубака, успевший пять лет в Европе повоевать с испанцами.";
...
void initNewMainCharacter()//инициализация главного героя
{
	...
	{        
		case "HeroType_1":
			SetSPECIAL(ch, 8, 6, 8, 5, 8, 8, 6);
			SetSelfSkill(ch, 15, 10, 10, 5, 15);
			SetShipSkill(ch, 5, 5, 5, 5, 5, 5, 5, 5, 5);
		...
		case "HeroType_2":
			SetSPECIAL(ch, 6, 5, 7, 6, 9, 7, 9);
			SetSelfSkill(ch, 5, 5, 5, 5, 15)
			SetShipSkill(ch, 5, 15, 5, 5, 15, 5, 5, 5, 5);
		...
		case "HeroType_3":
			SetSPECIAL(ch, 10, 4, 10, 5, 5, 10, 5);
			SetSelfSkill(ch, 5, 10, 15, 5, 10)
			SetShipSkill(ch, 5, 5, 5, 5, 5, 5, 15, 5, 5);
		...
		case "HeroType_4":
			SetSPECIAL(ch, 7, 8, 7, 5, 7, 7, 8);
			SetSelfSkill(ch, 5, 5, 5, 15, 5);
			SetShipSkill(ch, 5, 5, 15, 15, 5, 5, 5, 5, 5);
	...
	{
		ch.Ship.Type = GenerateShipExt((SHIP_FRIGATE_L + rand(0)), 0, ch);
		SetBaseShipData(ch);
		ch.Ship.Name = "Паллада";
...
void initMainCharacterItem()
{
	...
	{
		case "HeroType_1":
			itemID = GetGeneratedItem("blade_08");	// фехтовальщик
		...
		case "HeroType_2":
			itemID = GetGeneratedItem("blade_12");	// счетовод
            GiveItem2Character(ch, itemID);
            EquipCharacterbyItem(ch, itemID);
			AddItems(ch, "pistol1", 1);
			EquipCharacterbyItem(ch, "pistol1");			
			AddItems(ch, "bullet", 10);
			AddItems(ch, "gunpowder", 10);
			TakenItems(ch, "jewelry3", 5);
			TakenItems(ch, "jewelry42", 5);
			TakenItems(ch, "jewelry43", 5);
			TakenItems(ch, "jewelry46", 5);
         	ch.money = 10000;	
			TakenItems(ch, "gold_dublon", 100);	
			TakenItems(ch, "potion1", 3);
			AddItems(ch, "cirass5", 1);
			EquipCharacterbyItem(ch, "cirass5");
			TakenItems(ch, "obereg_8", 1);
		break;
		
		case "HeroType_3":
			itemID = GetGeneratedItem("blade_07");	// атлет
            GiveItem2Character(ch, itemID);
            EquipCharacterbyItem(ch, itemID);
		...
		case "HeroType_4":
			itemID = GetGeneratedItem("blade_16");	// стрелок
            GiveItem2Character(ch, itemID);
            EquipCharacterbyItem(ch, itemID);			
			AddItems(ch, "pistol5", 1);
			EquipCharacterbyItem(ch, "pistol5");			
			AddItems(ch, "bullet", 25);
			AddItems(ch, "gunpowder", 25);
			TakenItems(ch, "jewelry3", 1);
			TakenItems(ch, "jewelry42", 1);
			TakenItems(ch, "jewelry43", 2);
			TakenItems(ch, "jewelry46", 3);
