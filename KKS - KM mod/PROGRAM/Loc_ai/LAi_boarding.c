...
void LAi_StartBoarding(int locType, ref echr, bool isMCAttack)
{
	...
	if (CheckOfficersPerk(echr, "MusketsShoot") && IsFort == false) // для противника
	{
		...
		// --[ CSM
		if (csmCA(pchar, "CSM.BoardingCrewGodMode"))
			sLog("Мушкетным залпом противника, чудесным образом, не убито ни одного человека нашей команды!");
		else
		{
	...
	// Sticks -->
	SM_ShipCrewBeforeBoarding = mcrew; // запомним численность команды до начала рубки, после вернем обратно, т.к. они не участвуют
	// <-- Sticks
...
void LAi_EnableReload()
{
	...
	PostEvent("csmEventRefreshReload", 100);
	// CSM ]--
...
void LAi_BoardingGroupKill()
{
	...
	PostEvent("LAi_event_boarding_EnableReload", 3500);
}
...
string LAi_GetBoardingModel(ref rCharacter, ref ani)
{
	...
	if(sti(rCharacter.index) == GetMainCharacterIndex())
	{
		...
		// Sticksъ: для чего этот код? Для ГГ это и раньше отдельно делалось, через GenQuest.BoardCrewType
		...
		// > by Sticksъ
		if (CheckAttribute(pchar, "SquadType"))
			return ""; // тестовая фича - если есть отряд, то абордажим только с ним
		// by Sticksъ <
		...
		// > by Sticksъ - приортет ниже, чем у квестов (или выше, не важно... Главное, чтоб такой же был, как в блоке с мушкетерами)
		if (CheckAttribute(rCharacter, "GenQuest.BoardCrewType"))
		{
			CSM_GetModelByCrewType(sti(rCharacter.GenQuest.BoardCrewType), &model, &ani, false); // теперь вся сборка моделей тут
			return model;
		}
		// by Sticksъ <
...
{
	if (csmCA(mchr, "CSM.BoardingProcess") && sti(mchr.CSM.BoardingProcess) == 2)
		return false; // --> CSM
