...
float LAi_GetDamageAttackType(aref attack, aref enemy, string attackType, ref aBlade, bool isBlocked)
{
	...
	switch(attackType)
	{
		...
		case "round": 
			if(isBlocked && !csmCA(attack, "animal")) { kAttackDmg = 0.0; } // --> CSM
			else
			{
				if(sFencingType == "FencingL")
				{
...
float LAi_CalcDamageForMushket(aref attack, aref enemy, string attackType, bool isBlocked)
{
	...
	if(kAttackDmg > 0)  // оптимизация boal
	{
		...
		// --> Sticks: общий множитель урона при махании прикладом
		if (CheckAttribute(pchar, "CSM.GlobalGunDamageMultipler"))
...
{
	// --[ CSM
	if (csmCA(character, "CSM.NoEnergyDrain"))
		return 0.0;
	// CSM ]--
...
float LAi_GunCalcDamage(aref attack, aref enemy)
{
	...
	// --> Sticks: общий множитель урона от стрельбы
	if (CheckAttribute(pchar, "CSM.GlobalGunDamageMultipler"))
		dmg *= stf(pchar.CSM.GlobalGunDamageMultipler);
	// <-- Sticks
...
void LAi_ApplyCharacterAttackDamage(aref attack, aref enemy, string attackType, bool isBlocked)
{
	...
	if(LAi_IsDead(enemy))
	{
		...
		if (attack.model.animation == "mushketer")
		{
			if (CheckAttribute(enemy, "City")) AddCharacterExpToSkill(attack, LAi_GetMushketFencingType(attack), makefloat((10.0 + ((1 + re) / (1+ra))*6.5)/10));	
			else AddCharacterExpToSkill(attack, LAi_GetMushketFencingType(attack), makefloat(10.0 + ((1 + re) / (1+ra))*6.5));			
		}
		else
		{
			if (CheckAttribute(enemy, "City")) AddCharacterExpToSkill(attack, LAi_GetBladeFencingType(attack), makefloat((10.0 + ((1 + re) / (1+ra))*6.5)/10));
	...
	if (!noExp)
    {
		...
		if (attack.model.animation == "mushketer")
		{
			if (CheckAttribute(enemy, "City")) AddCharacterExpToSkill(attack, LAi_GetMushketFencingType(attack), Makefloat(exp*0.03));
			else AddCharacterExpToSkill(attack, LAi_GetMushketFencingType(attack), Makefloat(exp*0.3));
		}
		else
		{
			if (CheckAttribute(enemy, "City")) AddCharacterExpToSkill(attack, LAi_GetBladeFencingType(attack), Makefloat(exp*0.03));
			else AddCharacterExpToSkill(attack, LAi_GetBladeFencingType(attack), Makefloat(exp*0.3));
		}	
    }
...
void LAi_ApplyCharacterFireDamage(aref attack, aref enemy, float kDist)
{
	...
	if(LAi_IsDead(enemy))
	{
        ...
        if (CheckAttribute(enemy, "City"))
		{
			AddCharacterExpToSkill(attack, SKILL_PISTOL, makefloat(15.0 + ((1 + re) / (1+ra))*12.0)/10));
		}
		else
		{
	...
	if(!noExp)
    {
       if (CheckAttribute(enemy, "City")) AddCharacterExpToSkill(attack, SKILL_PISTOL, Makefloat(exp*0.2));
