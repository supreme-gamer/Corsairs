...
void CreateHabitues(aref loc)
{
	...
	if (!isLocationHasCitizens(loc.id))  // boal  если есть еще с того раза, но не нужно
	{
		if (CheckAttribute(loc, "fastreload"))
		{
			...
			// --[ CSM --> агенты-дипломаты в пиратских тавернах
			if (csmCA(pchar, "CSM.RelationAgent") && iNation == PIRATE && sti(Colonies[iColony].HeroOwn) != true)
			{
			...
			// офицеры <--
			if (csmCA(pchar, "CSM.TreasureMapTrader") || rand(10) == 1) // --> CSM // ugeen --> уменьшил вероятность встретить странную личность с картой до 1/20
			{ // карты кладов
